...
function VotePage() {
  ...
  const [step, setStep] = useState<
    'idle' | 'verifying' | 'verified' | 'voting' | 'success' | 'error'
  >('idle'); // idle | verifying | verified | voting | success | error
  ...
  const handleVerifyIdentity = async () => {
    ...
    try {
      ...
      const challengeRes = await axios.post(`${BACKEND_URL}/api/auth/challenge`, {
        studentId,
        walletAddress,
        voteType,
        voteId,
      ...
      await axios.post(`${BACKEND_URL}/api/auth/verify-challenge`, {
        studentId,
        walletAddress,
        signature,
        voteType,
      ...
      const isUserRejected =
        ...
        (typeof err?.message === 'string' &&
          (err.message.includes('ethers-user-denied') || err.message.includes('User rejected')));
      if (isUserRejected) {
  ...
  // ─── Derived ──────────────────────────────────────────────────────────────
  const allCandidates = [...(candidates.assigned ?? []), ...(candidates.selfNominated ?? [])];
        ...
        {step === 'success' && (
          <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm text-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-sm text-slate-500 font-medium mb-8 italic">
              Lá phiếu nặc danh của bạn đã được ghi nhận trên Blockchain.
            </p>
          ...
          <>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 md:p-10">
              <div className="flex items-center gap-3 mb-4">
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${isElection ? 'text-blue-600' : 'text-indigo-600'}`}
                >
            ...
            {!isActive && (
              <div className="p-6 bg-amber-50 border border-amber-200 rounded-2xl text-center">
                <p className="text-sm font-bold text-amber-700">
                  Cuộc bầu chọn này hiện không đang diễn ra (trạng thái:{' '}
                ...
                {step === 'idle' && (
                  <div className="bg-white border-2 border-slate-900 rounded-[1.5rem] p-8 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
                    <div className="flex items-start gap-4 mb-6">
                      <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center text-sm font-black shrink-0">
                        1
                      </div>
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 font-medium mt-1 leading-relaxed">
                          Ký xác thực bằng ví MetaMask đã đăng ký (
                          <span className="font-mono text-blue-600">
                            {walletAddress.slice(0, 6)}...{walletAddress.slice(-4)}
                          </span>
                          ) để chứng minh tư cách cử tri. Chữ ký này <strong>không phí gas</strong>.
                        </p>
                      </div>
                    </div>
                    {errorMsg && (
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-white border-2 border-slate-900 rounded-[1.5rem] p-8 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
                      <div className="flex items-start gap-4 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 font-medium mt-1">
                            Lá phiếu được mã hoá nặc danh trước khi gửi. Không ai có thể biết bạn
                            chọn ai.
                          </p>
                        </div>
                      </div>
                              ...
                              return allCandidates.map((c) => {
                                ...
                                const pct =
                                  totalCandidateVotes > 0
                                    ? Math.round((count / totalCandidateVotes) * 100)
                                    : 0;
                                return (
                                  <button
                                    key={c.id}
                                  ...
                                  >
                                    <div className="flex items-center gap-4">
                                      ...
                                      <div className="flex-1">
                                        <p className="font-black text-slate-900 uppercase tracking-tight mb-0.5">
                                          {c.name}
                                        </p>
                                        {c.bio && (
                      ...
                      {!isElection && event.options && (
                        <div className="space-y-3">
                          {(() => {
                            const totalOptionVotes = Object.values(optionVotes).reduce(
                            ...
                            return event.options.map((opt: string, idx: number) => {
                              ...
                              const pct =
                                totalOptionVotes > 0
                                  ? Math.round((count / totalOptionVotes) * 100)
                                  : 0;
                              return (
                                <button
                                  key={idx}
                    ...
                    <button
                      ...
                      disabled={
                        loading ||
                        (isElection ? !selectedCandidateId : selectedOptionIndex === null)
                      }
                ...
                {step === 'voting' && (
                  <div className="bg-white border border-slate-100 rounded-[2.5rem] p-12 text-center shadow-sm">
                    ...
                    <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest">
                      Mã hoá nặc danh & gửi lên Blockchain...
                    </p>
                  </div>
                )}
              </>
