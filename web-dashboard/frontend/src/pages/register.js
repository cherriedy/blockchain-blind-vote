...
export default function RegisterPage() {
    ...
    const handleSubmit = () => {
        ...
        if (!trimmedWallet) {
            setMsg({ type: 'error', text: 'Vui lòng kết nối ví MetaMask trước khi tiếp tục.' });
            return;
        }

        localStorage.setItem('studentId', trimmedStudentId.toUpperCase());
        localStorage.setItem('walletAddress', trimmedWallet.toLowerCase());
    ...
    return (
        <>
            <Head>
                <title>Đăng nhập — Hệ thống Bầu cử</title>
            </Head>
        ...
        {/* Message */}
                        {msg.text && (
                            <div className={`mt-6 p-4 rounded-xl border-2 flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-500 ${
