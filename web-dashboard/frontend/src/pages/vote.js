...
// import { io } from 'socket.io-client'; // [TẠM BỎ WEBSOCKET]
...
export default function VotePage() {
    ...
    // ── URL params ──
    const [voteType, setVoteType] = useState('');  // 'election' | 'poll'
    ...
    const [optionVotes, setOptionVotes] = useState({});    // { [optionIndex]: count }
    const [candidateVotes, setCandidateVotes] = useState({}); // { [candidateId]: count }
    ...
    // ── Flow state ──
    const [step, setStep] = useState('idle'); // idle | verifying | verified | voting | success | error
    ...
    useEffect(() => {
        ...
        const { voteType: qt, voteId: qid } = router.query;
        if (!qt || !qid) {
        ...
        setVoteId(qid);
        fetchEvent(qt, qid);
    }, [fetchEvent, router, router.isReady]);

    const fetchEvent = useCallback(async (type, id) => {
        setLoading(true);
        try {
            if (type === 'election') {
                ...
                const counts = await Promise.all(
                    allCands.map(c =>
                        axios.get(`${BACKEND_URL}/api/voting/election/${id}/votes/${c.id}`)
                            .then(r => [c.id, Number(r.data.count)])
                            .catch(() => [c.id, 0])
                    )
                );
                ...
                const counts = await Promise.all(
                    opts.map((_, idx) =>
                        axios.get(`${BACKEND_URL}/api/voting/poll/${id}/votes/${idx}`)
                            .then(r => [idx, Number(r.data.count)])
                            .catch(() => [idx, 0])
                    )
                );
    ...
    return (
        <>
            <Head>
                <title>Bỏ phiếu — Hệ thống Bầu cử</title>
                <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700;800;900&display=swap" rel="stylesheet" />
                <style>{`.vote-page { font-family: 'Outfit', sans-serif !important; }`}</style>
            </Head>
                                        ...
                                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                            ...
                                            <div className="bg-white border-2 border-slate-900 rounded-[1.5rem] p-8 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
                                                ...
                                                {!isElection && event.options && (
                                                    <div className="space-y-3">
                                                        {(() => {
                                                            const totalOptionVotes = Object.values(optionVotes).reduce((a, b) => a + b, 0);
                                                            return event.options.map((opt, idx) => {
