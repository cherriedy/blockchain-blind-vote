...
import { Voter, EventVoter } from '@/lib/types/admin';
...
export default function VoterManagerSection({
    ...
    const searchVoter = async (val: string) => {
        if (val.length > 1) {
            ...
            const filtered = res.data.filter((v: Voter) =>
                ...
                {voters.length > 0 && (
                    <div className="absolute z-20 w-full mt-1 bg-white border rounded shadow max-h-40 overflow-auto">
                        {voters.map(v => (
                            ...
                            >
                                <span> {v.name}
                                    <small className="text-slate-400 ml-1">
                                        ({v.studentId})
                                    </small>
                                </span>
            ...
            <div className="flex flex-wrap gap-2">
                {addedVoters.map(v => (
                    <div key={v.id} className="bg-blue-100 px-2 py-1 text-xs">
                        ...
                        <button
                            onClick={() =>
                                setAddedVoters(addedVoters.filter(x => x.id !== v.id))
                            }
                        >
                            ×
                        </button>
                ...
                {objectVoters.length === 0 ? (
                    ...
                    <Card key={v.voterId}>
                        <CardContent className="flex justify-between p-3">
                            <span> {v.voter.studentId} - {v.voter.name} </span>
