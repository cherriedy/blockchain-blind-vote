...
export default function CandidateManagerSection({ electionId, form, setForm }: Props) {
  ...
  const searchCandidate = async (val: string) => {
    if (val.length > 1) {
      ...
      const filtered = res.data.filter(
        (c: Candidate) =>
          !form.candidateIds.includes(c.id) && !selectedCandidates.some((sc) => sc.id === c.id),
      );
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm font-semibold text-slate-700">Danh sách ứng cử viên</p>
      ...
      <div className="flex flex-wrap gap-2">
        {selectedCandidates.map((c) => (
          <div key={c.id} className="bg-blue-100 px-2 py-1 text-xs rounded">
            {c.name}
            <button
              onClick={() => setSelectedCandidates((prev) => prev.filter((i) => i.id !== c.id))}
            >
              ×
            </button>
          </div>
          ...
          assignedCandidates.map((c) => (
            <Card key={c.id}>
              <CardContent className="flex justify-between p-3">
                <span>{c.name}</span>
                <Button variant="destructive" size="sm" onClick={() => handleRemoveCandidate(c.id)}>
                  Xóa
                </Button>
              </CardContent>
            </Card>
      ...
      {/* SELF NOMINATION */}
      <div className="space-y-2">
        <p className="text-xs font-semibold text-orange-500">Chờ duyệt</p>
        {selfNominees.length > 0 ? (
          selfNominees.map((c) => (
            <Card
              key={c.id}
              onClick={() => fetchNomineeDetail(c.id)}
            ...
            <div className="space-y-4">
              ...
              {selectedNominee.adminNotes && (
                <div>
                  ...
                  <p className="text-sm text-red-500">{selectedNominee.adminNotes}</p>
                </div>
              )}
            </div>
          )}
