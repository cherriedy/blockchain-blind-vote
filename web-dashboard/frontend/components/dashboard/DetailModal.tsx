  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className={`p-7 md:p-10 border-b border-slate-100 flex justify-between items-start ${isElection ? 'bg-blue-50/40' : 'bg-indigo-50/40'}`}
        >
          <div className="flex-1 mr-4">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight leading-tight">
              {item.name}
            </h3>
          </div>
          <button
          ...
          <div className="flex-1 p-7 md:p-10 overflow-y-auto border-b md:border-b-0 md:border-r border-slate-100 space-y-5">
            ...
            {!isElection && item.question && (
              <div>
                ...
                <p className="text-base font-bold text-slate-700 italic bg-indigo-50 p-4 rounded-xl">
                  {item.question}
                </p>
              </div>
            )}
          ...
          <div className="flex-1 p-7 md:p-10 overflow-y-auto space-y-5">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold text-slate-400">
                      {(candidates.assigned?.length ?? 0) + (candidates.selfNominated?.length ?? 0)}{' '}
                      tổng
                    </span>
                  </div>
                  {totalVotes > 0 && (
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-xl p-4">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-black text-slate-700">
                  {item.isAutomatic ? 'Tự động' : 'Thủ công'}
                </p>
              </div>
            </div>
          </div>
