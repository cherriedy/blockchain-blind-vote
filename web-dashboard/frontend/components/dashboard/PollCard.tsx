...
export default function PollCard({ poll, onClick }: PollCardProps) {
  const totalVotes = Array.isArray(poll.votes) ? poll.votes.reduce((a, b) => a + Number(b), 0) : 0;
        ...
        {poll.options && poll.options.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {poll.options.slice(0, 3).map((opt, i) => (
              <span
                key={i}
                className="px-2.5 py-1 bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-wide rounded-lg border border-slate-100"
              >
                {opt}
              </span>
            ))}
